...
class CacheService(BaseService):
    ...
    def get(self, task: TaskModel) -> TaskModel:
        if task.content_hash:
            ...
            if task_found:
                logger_abrege.debug(f"[task_id {task.id}] Retrieving task from cache with content_hash: {task.content_hash}")
                return task_found
        raise NotImplementedError(f"Task with content_hash {task.content_hash} not found in cache.")
