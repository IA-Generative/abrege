...
from src.schemas.result import ResultModel, OcrPage, OcrBbox
...
class OCRMIService(BaseService):
    ...
    def task_to_text(self, task: TaskModel, **kwargs) -> TaskModel:
        ...
        for batch in batch_list(images, batch_size=min(10, len(images))):
            ...
            logger.debug(
                f"batch size {len(batch)} get {task_ids} as ocr id", extra=extra_log
            )
                ...
                for task_id_tmp in task_ids:
                    if current_index not in tmp_page_ocr_index:
                        ...
                        if status in task_finish_on_error:
                            logger.error(
                                f"{task_id_tmp} is on error - {status}", extra=extra_log
                            )
                ...
                task.output.ocr_pages = [
                    OcrPage(
                        ...
                        boxes=[
                            ...
                            for b in raw_page.boxes
                        ],
                    )
                    for raw_page in (page_raw_index[j] for j in sorted(page_raw_index))
                ]
                task = self.update_task(
                    task=task, status=TaskStatus.IN_PROGRESS, result=task.output
                )
