...
class VideoTranscriptionService(VideoBaseService):
    ...
    def task_to_text(self, task: TaskModel, **kwargs) -> TaskModel:
        if not os.path.exists(task.input.file_path):
            raise FileExistsError(f"video {task.input.file_path} don't exist")

        tmp_file = f"{uuid4().hex}.wav"
        if folder_dest:
            tmp_file = os.path.join(folder_dest, tmp_file)

        extraire_audio(task.input.file_path, tmp_file)
        if not os.path.exists(tmp_file):
            raise FileExistsError(f"audio {tmp_file} create from video {task.input.file_path} don't exist")
        task_audio = task.model_copy()
        task_audio.input = DocumentModel(
            created_at=task.input.created_at,
            file_path=tmp_file,
            raw_filename=task.input.raw_filename,
            content_type="audio/wav",
            ext="wav",
            size=task.input.size,
        )
        task_audio = self.audio_service.task_to_text(task=task_audio)
        task.output = task_audio.output
