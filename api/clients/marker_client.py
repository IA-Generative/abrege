...
class MarkerAPIClient:
    ...
    async def acheck_health(self) -> HealthResponse:
    ...
    def _log_server_info(self, health_data: HealthResponse):
        if health_data.type == ServerType.simple:
            ...
            logger.info(f"Connected to a distributed server with {health_data.workers} workers")

    def _convert_endpoint(self):
        return "/convert" if self.server_type == ServerType.simple else "/celery/convert"

    def _batch_convert_endpoint(self):
        return "/batch_convert"

    def load_data(self, file_paths: Union[str, List[str]], show_progress: bool = False) -> Union[ConversionResponse, BatchConversionResponse]:
    ...
    def _convert_single(self, file_path: str) -> ConversionResponse:
        with open(file_path, "rb") as file:
            ...
            response = self.session.post(f"{self.base_url}{self._convert_endpoint()}", files=files)
        response.raise_for_status()
        logger.info(f"Successfully converted {file_path}")
        return ConversionResponse(**response.json())

    def _convert_batch(self, file_paths: List[str], show_progress: bool) -> BatchConversionResponse:
        files = []
        ...
        response = self.session.post(f"{self.base_url}{self._batch_convert_endpoint()}", files=files)
        response.raise_for_status()
        logger.info("Batch conversion request successful")
        return BatchConversionResponse(**response.json())

    async def aload_data(self, file_paths: Union[str, List[str]], show_progress: bool = False) -> Union[ConversionResponse, BatchConversionResponse]:
        if isinstance(file_paths, str):
    ...
    async def _aconvert_single(self, file_path: str) -> ConversionResponse:
        ...
        async with self.async_session.post(f"{self.base_url}{self._convert_endpoint()}", data=data) as response:
            response.raise_for_status()
            logger.info(f"Successfully converted {file_path} asynchronously")
            return ConversionResponse(**(await response.json()))

    async def _aconvert_batch(self, file_paths: List[str], show_progress: bool) -> BatchConversionResponse:
        data = aiohttp.FormData()
        async for file_path in atqdm(file_paths, desc="Preparing files", disable=not show_progress):
            data.add_field("pdf_files", open(file_path, "rb"))
            logger.info(f"Prepared file: {file_path}")

        logger.info("Sending async batch conversion request")
        async with self.async_session.post(f"{self.base_url}{self._batch_convert_endpoint()}", data=data) as response:
            response.raise_for_status()
    ...
    async def aget_result(self, task_id: str) -> ConversionResponse:
        if self.server_type != ServerType.distributed:
            raise ValueError("aget_result is only available for distributed server type")
        logger.info(f"Getting result asynchronously for task {task_id}")
        async with self.async_session.get(f"{self.base_url}/celery/result/{task_id}") as response:
            response.raise_for_status()
            logger.info(f"Successfully retrieved result asynchronously for task {task_id}")
            return ConversionResponse(**(await response.json()))

    def get_batch_result(self, task_id: str) -> BatchConversionResponse:
        if self.server_type != ServerType.distributed:
            raise ValueError("get_batch_result is only available for distributed server type")
    ...
    async def aget_batch_result(self, task_id: str) -> BatchConversionResponse:
        if self.server_type != ServerType.distributed:
            raise ValueError("aget_batch_result is only available for distributed server type")
        logger.info(f"Getting batch result asynchronously for task {task_id}")
        async with self.async_session.get(f"{self.base_url}/batch_convert/result/{task_id}") as response:
            response.raise_for_status()
            logger.info(f"Successfully retrieved batch result asynchronously for task {task_id}")
            return BatchConversionResponse(**(await response.json()))
...
async def main():
    async with MarkerAPIClient("http://localhost:8080") as client:
        ...
        batch_result = client.load_data(["./file1.pdf", "./file2.pdf", "./file3.pdf"], show_progress=True)
        print(batch_result)

        # Asynchronous batch conversion with progress
        async_batch_result = await client.aload_data(["./file1.pdf", "./file2.pdf", "./file3.pdf"], show_progress=True)
